import React from 'react'
import styled from 'styled-components';
import {useAuth} from '../context/AuthContext';
import {useHistory} from 'react-router-dom';
import {auth} from '../firebase/firebaseConfig';

export const BurguerMenu = ({changeMenuBurguer}) => {

    const {user} = useAuth();
    const history = useHistory();

    const handleClick = () => {
        history.push('/perfil')
    }
    const handleLogoutClick = async () => {
        try {
            await auth.signOut();
            history.push('/presentacion')
        } catch (error) {
            
        }
    }

    return (
        <Menu>
            <Header>
                <p>Informacion de la cuenta</p>
                <CloseMenu onClick={() => changeMenuBurguer(false)}><i className="fas fa-times"></i></CloseMenu>
            </Header>
            <Container>
                {
                    user.photoURL ? <Img onClick={handleClick} src={user.photoURL} alt=""/> : <Img onClick={handleClick} src='data:image/jpeg;base64,' alt=""/> 
                }
                
                <h4>{user.displayName}</h4>
                <DFlex>
                    <p>0 siguiendo</p>
                    <p>0 seguidores</p>
                </DFlex>
                <Button onClick={handleClick}><i className="far fa-user"></i>Perfil</Button>
                <Button onClick={handleLogoutClick}>Cerrar sesión</Button>
            </Container>
            
        </Menu>
    )
}
const Menu = styled.div`
    position:fixed;
    top:0;
    bottom:0;
    color:white;
    background:rgb(81, 81, 81);
    height:100vh;
    width:75%;

    @media(min-width:768px){
        width:40%;
    }
`;
const Header = styled.div`
    display:flex;
    justify-content:space-between;
    border-bottom:solid 1px lightgray;
    max-width:100%;
    padding:20px;
    &>p{
        font-weight:bold;
    }
`;
const Container = styled.div`
    padding:20px;
`;
const Img = styled.img`
    border-radius:100%;
    width:50px;
    height:50px;
`;
const DFlex = styled.div`
    display:flex;
    justify-content:space-between;
    margin:1rem 0;
`;
const Button = styled.button`
    display:block;
    margin-bottom:.7rem;
    background:transparent;
    border:none;
    color:white;
    outline:none;
    font-size:15px;

    & > i {
        margin-right:5px;
    }
`;
const CloseMenu = styled.button`
    background:transparent;
    border:none;

    & > i {
        color:white;
        font-size:20px;
    }
`;