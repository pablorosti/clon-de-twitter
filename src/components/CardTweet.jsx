import React, {useState} from 'react'
import styled from 'styled-components';
import {Link} from 'react-router-dom';
import {addLike} from '../firebase/addLike';
import {removeLike} from '../firebase/removeLike';
import {addRetweet} from '../firebase/addRetweet';
import {removeRetweet} from '../firebase/removeRetweet';
import {useAuth} from '../context/AuthContext';
import {useTimeAgo} from '../hooks/useTimeAgo';
import {useGetCommentTweetForId} from '../hooks/useGetCommentTweetForId';
//import {deleteTweet} from '../firebase/deleteTweet';
//import {useGetTweetsUser} from '../hooks/useGetTweetsUser';


export const CardTweet = ({tweet, name, id, idUnico, 
    changeDeleteModal, photo, date, image, likes, retweet, verified, setIdDeleted}) => {

    const {user} = useAuth();
    const timeAgo = useTimeAgo(date)
    const [amountComment] = useGetCommentTweetForId(id);
     
    //function to add and remove a like of the tweet.
    const [contLike, setContLike] = useState(0);
    const handleLikeClick = () => {
        
        if(contLike === 0){
            addLike(id, likes);
            setContLike(contLike + 1);
        }else{
            removeLike(id, likes)
            setContLike(contLike - 1)
        }   
    }

    //function to add and remove a retweet of the tweet.
    const [contRetweet, setContRetweet] = useState(0);
    const handleRetweetClick = () => {

        if(contRetweet === 0){
            addRetweet(id, retweet);
            setContRetweet(contRetweet + 1);
        }else{
            removeRetweet(id, retweet)
            setContRetweet(contRetweet - 1)
        }
    }

    const handleClick = () => {
        changeDeleteModal(true);
        setIdDeleted(id)
    }

    return (
        < >
            <Card>
                {
                    photo  
                        ? <Img src={photo} alt=""/> 
                        : <Img src='data:image/jpeg;base64,' alt=""/>
                }
                
                <div>
                    <DFlex>
                        <div>
                            <Name>{name}</Name>
                            {verified ? <Verified className="fas fa-check-circle"></Verified> : null}
                            <P>.</P>
                            <Date>
                                <Time to={`/tweet/${id}`}>
                                    {timeAgo}
                                </Time>
                            </Date>
                            
                        </div>
                        {
                            user ? null : <Button onClick={handleClick}><i className="fas fa-ellipsis-h"></i></Button>
                        }
                        
                    </DFlex>
                    
                    <Tweet>{tweet}</Tweet>
                    {
                        image ? <Image src={image} alt=""/> : null
                    }
                    
                    <DFlex>
                        <ButtonComment to={`/tweet/${id}`}><i className="far fa-comment"></i>{amountComment.length !== 0 ? <Amount>{amountComment.length}</Amount>: null }</ButtonComment>
                        <ButtonRetweet><i onClick={handleRetweetClick} className="fas fa-retweet"></i>{retweet ? <Amount> {retweet}</Amount> : null}</ButtonRetweet>
                        <ButtonLike ><i onClick={handleLikeClick} className="far fa-heart"></i>{likes ? <Amount> {likes}</Amount> : null}</ButtonLike>
                        <Button><i className="fas fa-share"></i></Button>
                    </DFlex>
                </div>
                
            </Card>
        </>
    )
}

const Card = styled.div`
    border-bottom:solid 1px lightgray;
    width:100%;
    padding-top:10px;
    padding-bottom:10px;
    display:grid;
    grid-template-columns:1fr 7fr .3fr;
    gap:10px;

    :hover{
        background:rgb(243, 241, 241);
    }
`;
const Img = styled.img`
    border-radius:100%;
    width:40px;
    height:40px;
    margin-top:10px;
    margin-left:10px;
`;
const Name = styled.h3`
    font-size:1rem;
    text-transform:capitalize;
    margin-bottom:5px;
`;
const Tweet = styled.p`
    font-size:1rem;
    margin-bottom:13px;
    color:var(--gray);
`;
const DFlex = styled.div`
    display:flex;
    justify-content:space-between;
    align-items:center;

    & > i {
        color:gray;
        cursor:pointer;
    }
    & > div {
        display:flex;
    }
`;
const ButtonComment = styled(Link)`
    background:transparent;
    outline:none;
    border:none;
    cursor:pointer;
    color:gray;
    display:flex;
    align-items:center;
    text-decoration:none;
    font-size:13.5px;
`;
const ButtonRetweet = styled.button`
    background:transparent;
    outline:none;
    border:none;
    cursor:pointer;
    color:gray;
    display:flex;
    align-items:center;

    :hover{
        color:rgb(51, 182, 51);
    }
`;
const ButtonLike = styled.button`
    background:transparent;
    outline:none;
    border:none;
    cursor:pointer;
    color:gray;
    display:flex;
    align-items:center;

    
`;
const Button = styled.button`
    background:transparent;
    outline:none;
    border:none;
    cursor:pointer;
    color:gray;
`;
const Date = styled.p`
    margin-left:10px;
    color:gray;
    font-size:15px;
    margin-top:1px;
`;
const P = styled.p`
    margin-left:5px;
    
`;
const Image = styled.img`
    width:100%;
    margin-bottom:10px;
    max-height:auto;
    border-radius:10px;
`;

const Time = styled(Link)`
    text-decoration:none;
    color:gray;
`;
const Amount = styled.p`
    margin-left:3px;
`;
const Verified = styled.i`
    color:var(--primary);
    font-size:13px;
    margin-top:4px;
`;